<script lang="ts">
	...
	const exportFilteredRows = () => {
		...
		anchor.download = `penerbitan-persetujuan-lingkungan-${formatIsoDate(new Date())}.csv`;
...
<svelte:head>
	<meta
		name="description"
		content="Halaman pengumuman penerbitan persetujuan lingkungan berisi daftar instansi, kegiatan, nomor SK, dan tanggal penerbitan."
	/>
</svelte:head>
...
<section class="relative overflow-hidden bg-(--canvas) pt-28 pb-16 sm:pt-32 sm:pb-20">
	<div class="nav-shell nav-shell-desktop-spacious relative">
		<div hidden data-universal-search-index="publishing-rows">{publishingRowsSearchIndexText}</div>
		...
		<div class="mt-0 space-y-4">
			<div class="space-y-2.5 border-b border-[#e6ebf2] pb-3.5">
				<div class="flex flex-col gap-2.5 md:flex-row md:items-center md:justify-between">
					<label for="search-penerbitan-perling" class="flex h-11 w-full items-center gap-3 rounded-lg border border-[#cfd7e3] bg-[#ffffff] px-3.5 md:max-w-120 md:px-4">
						<Search class="h-4.5 w-4.5 text-(--muted)" strokeWidth={2} aria-hidden="true" />
						<input
							id="search-penerbitan-perling"
							type="text"
					...
					<div class="grid grid-cols-2 gap-2 sm:flex sm:items-center md:shrink-0">
						<button
							...
							aria-controls="penerbitan-perling-filter-panel"
			...
			{#if isFilterPanelOpen}
				<div id="penerbitan-perling-filter-panel" bind:this={filterPanelElement} class="rounded-lg border border-[#d7dee8] bg-[#ffffff] p-3 sm:p-4">
		...
		{#if totalFilteredRows > 0}
			<div class="mt-6 pt-2">
				<nav aria-label="Navigasi halaman penerbitan persetujuan lingkungan" class="mx-auto flex flex-wrap items-center justify-center gap-1.5">
