...
export const load: PageServerLoad = async ({ locals, url, parent, depends }) => {
	depends('admin:pengumuman-integrasi')

	const filters = readFilters(url.searchParams)
	const adminData = await parent()

	if (!adminData.supabaseAvailable || !locals.supabase) {
		logAdminLoad('admin/pengumuman/integrasi/+page.server', { state: 'unavailable' })
		return {
			unavailable: true,
	...
	if (!adminData.isAdmin) {
		logAdminLoad('admin/pengumuman/integrasi/+page.server', { state: 'forbidden', role: adminData.role })
		return {
			unavailable: false,
	...
	try {
		...
		return {
			unavailable: false,
			requiresSupabaseAuth: false,
			errorMessage: error instanceof Error ? error.message : 'Gagal memuat data penerbitan integrasi',
			filters,
			summary: adminData.summary ?? EMPTY_SUMMARY,
			result: createEmptyResult(filters.page, filters.pageSize)
		}
